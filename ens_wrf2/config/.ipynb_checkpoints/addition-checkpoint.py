...
pcp_correct = f2_pcp+f1_pcp
tmean_correct = f2_tmean+f1_tmean
trange_correct = f2_trange+f1_trange
...
with nc.Dataset(inFile1) as src:
    with nc.Dataset(outFile, "w") as dst:
        ...
        dst.variables['pcp'][:] = pcp_correct
        dst.variables['t_mean'][:] = tmean_correct  
        dst.variables['t_range'][:] = trange_correct 
